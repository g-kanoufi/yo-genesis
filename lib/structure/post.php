...
add_filter( 'the_content', 'yg_remove_ptags_on_images' );
/**
 ...
 * @since 1.x
 */
function yg_remove_ptags_on_images( $content ){

	return preg_replace( '/<p>\s*(<a .*>)?\s*(<img .* \/>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $content );

}

add_filter( 'gallery_style', 'yg_gallery_style' );
/**
 * Remove the injected styles for the [gallery] shortcode
 *
 * @since 1.x
 */
function yg_gallery_style( $css ) {

	return preg_replace( "!<style type='text/css'>(.*?)</style>!s", '', $css );

}

// add_filter( 'the_content_more_link', 'yg_more_tag_excerpt_link' );
/**
 ...
 * @since 2.0.16
 */
function yg_more_tag_excerpt_link() {

	return ' <a class="more-link" href="' . get_permalink() . '">'.__("Read more &rarr;", $themename ).'</a>';

}

// add_filter( 'excerpt_more', 'yg_truncated_excerpt_link' );
// add_filter( 'get_the_content_more_link', 'yg_truncated_excerpt_link' );
/**
 ...
 * @since 2.0.16
 */
function yg_truncated_excerpt_link() {
...
// add_filter( 'genesis_post_info', 'yg_post_info' );
/**
...
// add_filter( 'genesis_post_meta', 'yg_post_meta' );
/**
 ...
 * @since 2.0.0
 */
function yg_post_meta() {

	return '[post_categories before="Filed Under: "] [post_tags before="Tagged: "]';

}

// add_filter ( 'genesis_prev_link_text' , 'yg_prev_link_text' );
/**
 ...
 * @since 2.0.0
 */
function yg_prev_link_text( $text ) {

    return html_entity_decode('&#10216;') . ' ';

}

// add_filter ( 'genesis_next_link_text' , 'yg_next_link_text' );
/**
 ...
 * @since 2.0.0
 */
function yg_next_link_text( $text ) {
